...
> **版本目标**：v0.3.0（导出增强：血缘树可导出为 Markdown）
> **文档状态**：已通过可行性自审（v0.1 修订版）＋ 端到端实测复核（v0.1.2 / v0.2.0 / v0.3.0）＋ 纯函数单元测试
> **最后更新**：2026-09-11
> **修订说明**：相较初版，已修正 3 处技术误区、落实必改清单、按评审结论重排里程碑；v0.1.1 做界面重做与配置收敛；v0.1.2 用无头浏览器端到端实测抓出并修掉 5 类真实缺陷；v0.2.0 交付 Side Panel 与时间轴视图，并把渲染逻辑抽成两入口共用的层。详见「修订摘要」「自检清单」与「十四至十七」。
...
│   ├── exporter.js                  # 导出层：Markdown/JSON 序列化（纯函数）+ 下载
...
> `checkrefs.mjs`（引用完整性）、`unit.mjs`（纯函数单测，不依赖浏览器）、`e2e.mjs`（无头 Edge + CDP 端到端）。
...
| ~~v0.3~~ | ~~导出功能增强~~ | ✅ v0.3.0 已交付 **Markdown**。原列的「Obsidian 格式」判定为不需要 —— Markdown + YAML front-matter 已天然兼容，没必要为此多一个开关。JSON 保留（可回读/做分析） |
...
| 导出：血缘树可导出为 Markdown | ✅ | **单测**：嵌套缩进 / URL 尖括号包裹 / 标题元字符转义 / front-matter 条数与链数 / 空数据提示；**e2e**：按钮存在、无记录时禁用、点击不抛异常 |
| 导出：JSON 可回读 | ✅ | 单测：`JSON.parse` 回读后字段完整 |
...
## 十七、v0.3.0 交付：血缘树导出为 Markdown

### A. 交付内容

| 项 | 说明 |
|----|------|
| **`lib/exporter.js`** | 导出层。`toMarkdown` / `toJSON` 是**纯函数**（不碰 DOM），`download` / `defaultName` 才碰浏览器 —— 拆开是为了让序列化逻辑可以在 node 里直接单测 |
| **Markdown 格式** | YAML front-matter（`generator` / `exported_at` / `records` / `chains`，对 Obsidian 友好）+ 嵌套列表（两空格缩进表示层级）。链接用 CommonMark 的尖括号形式 `[标题](<URL>)`，URL 含空格/括号也不会把链接截断；标题里的 `[` `]` `*` `` ` `` 等元字符做转义。搜索节点显示引擎名，未关闭记录带「打开中」标记 |
| **导出「当前所见」** | 导出与视图共用同一份**过滤后**的集合 —— 搜过/筛过再导出，拿到手的就是屏幕上那部分，不会所见非所得 |
| **底栏重排** | `[导出 JSON] [导出 MD] [🗑]`。清空改成图标按钮：它是破坏性操作，不该和两个导出按钮平起平坐抢宽度。无记录时两个导出按钮禁用（不导出空文件） |
| **`.workbuddy/tools/unit.mjs`** | 项目**第一批单元测试**（33 项）：`buildTree` 的环截断与孤儿提升、`applyFilter` 的参数语义、时间格式化、Markdown 全部细节、JSON 回读 |

### B. 三个设计决策

1. **不做「Obsidian 格式」这个第三选项。** 原计划列了 Markdown / JSON / Obsidian 三种。实际做时发现 Markdown + YAML front-matter 已经天然兼容 Obsidian，为它单独做一个开关是伪需求 —— 删掉。
2. **不引入 `downloads` 权限。** 下载用 `Blob + <a download>`（扩展页面里实测可用）。为一个下载加一条权限违背最小权限原则。
3. **下载不用 `downloads` API 的另一个代价是拿不到下载完成回调。** 无所谓 —— 我们不关心下载进度，只关心文件生成了。

### C. 这轮补上的短板：项目有了第一批单元测试

此前所有验证都压在 e2e 上。e2e 能证明「扩展装进浏览器后行为正确」，但**跑得慢、断言粒度粗、且只能测到浏览器里能触发的路径**。序列化这种纯逻辑更适合单测：毫秒级、失败时直接给出期望与实际值。

拆出纯函数的直接收益：`toMarkdown` 在 node 里直接 `import` 就能测，不用起浏览器。Markdown 的每个细节（缩进、转义、front-matter、空态）各占一条断言，坏了立刻知道是哪一条。

> **方法论**：判断一段逻辑该放 e2e 还是单测，看它是否碰浏览器 —— 碰 chrome.\*/DOM 的交给 e2e，纯算法/序列化的交给单测。两端都不碰的项目就不该存在。

### D. 验收结果

| 工具 | 结果 |
|------|------|
| `checkrefs.mjs` | **69 条**引用全部命中（新增 exporter 与底栏按钮后从 64 涨到 69） |
| `unit.mjs` | **33 项**全绿 |
| `e2e.mjs` | **68 项**全绿（在 v0.2.0 的 63 项之上新增：导出 MD 按钮存在、有记录时可用、点击不抛异常、无记录时禁用） |
